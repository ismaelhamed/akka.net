//-----------------------------------------------------------------------
// <copyright file="DurableStateActorImpl.cs" company="Akka.NET Project">
...
{
    [InternalApi]
    public abstract partial class DurableStateActorImpl : ActorBase, IWithUnboundedStash, IDurableStateActor
    {
        private readonly IDurableStateUpdateStore<object> _durableStateUpdateStore;
        private readonly IStash _internalStash;
        private DurableStateActorState _currentState;
        ...
        /// Holds user-supplied callbacks for persist calls
        private readonly LinkedList<StashingHandlerInvocation> _pendingInvocations = new LinkedList<StashingHandlerInvocation>();
        ...
        public DurableStateSettings Settings { get; }
        ...
        protected DurableStateActorImpl()
        {
            Extension = Persistence.Instance.Apply(Context.System);
            Settings = DurableStateSettings.Create(Context.System, "");
            _durableStateUpdateStore = DurableStateStoreRegistry.Get(Context.System)
        ...
        /// <summary>
        /// Asynchronously persists the <paramref name="state"/>. On successful persistence, the <paramref name="handler"/>
        /// is called with the persisted event. This method guarantees that no new commands will be received by a persistent actor
        /// between a call to <see cref="Persist{TState}(TState,System.Action{TState})"/> and execution of its handler. It also
        /// holds multiple persist calls per received command. Internally this is done by stashing. The stash used
        /// for that is an internal stash which doesn't interfere with the inherited user stash.
        ///
        ///
        /// A state <paramref name="handler"/> may close over eventsourced actor state and modify it.
        /// Sender of the persistent state is considered a sender of the corresponding command.
        /// That means one can respond to sender from within a state handler.
        ///
        ///
        /// Within an event handler, applications usually update persistent actor state using
        /// persisted new state data, notify listeners and reply to command senders.
        ///
        ///
        /// If persistence of the state fails, <see cref="OnPersistFailure" /> will be invoked and the actor will
        /// unconditionally be stopped. The reason that it cannot resume when persist fails is that it
        /// is unknown if the state was actually persisted or not, and therefore it is in an inconsistent
        ...
        public void Persist<TState>(TState state, Action<TState> handler)
        {
            if (IsRecovering)
            {
                throw new InvalidOperationException(
                    "Cannot persist during recovery. State can be persisted when receiving RecoveryCompleted or later.");
            }
        ...
        /// <summary>
        /// Called whenever the state replay succeeds.
        /// </summary>
        ...
        {
            Log.Error(cause, "Failed to persist state [{0}] for persistenceId [{1}].",
                state.GetType(), PersistenceId);
        }

        /// <summary>
        /// Called when the journal rejected <see cref="Persist{TState}(TState, Action{TState})"/> of state.
        ...
        private class InternalStashAwareStash : IStash
        {
            ...
            public void UnstashAll(Func<Envelope, bool> predicate) => _userStash.UnstashAll(predicate);

            public IEnumerable<Envelope> ClearStash() => _userStash.ClearStash();
    ...
    [InternalApi]
    public sealed class GetSuccess
    {
    ...
    [InternalApi]
    public sealed class GetFailure
    {
    ...
    [InternalApi]
    public sealed class UpsertSuccess
    {
    ...
    [InternalApi]
    public sealed class UpsertFailure
    {
        public Exception Cause { get; }
        public UpsertFailure(Exception cause) => Cause = cause;
    }
